...
class Game{
	constructor(){
		...
		this.engineSound = document.getElementById('engineSound');
...
this.rightKeyPressed = false;
		...
		document.addEventListener('keydown', function(event) {
			switch (event.code) {
				case 'KeyW':
					...
					game.forwardKeyPressed = true;
					break;
				case 'KeyS':
					game.backwardKeyPressed = true;
					break;
				case 'KeyA':
		...
		document.addEventListener('keyup', function(event) {
			switch (event.code) {
				case 'KeyW':
					game.forwardKeyPressed = false;
					break;
				case 'KeyS':
					game.backwardKeyPressed = false;
					break;
				case 'KeyA':
	...
	loadAssets(){
		...
		loader.load( '../assets/chess town3.fbx', 
		function ( object ){
	...
	initPhysics(){
		...
		const chassisShape = new CANNON.Box(new CANNON.Vec3(1, 0.3, 1));
	...
	// Make sure to include the three-geometry-extras library in your HTML file before this script
// <script src="path/to/three-geometry-extras.js"></script>
...
createColliders(){
	...
	this.assets.children.forEach(function(child){
		if (child.isMesh && child.name.includes("solid")){
			child.visible = false
    ...
    updateDrive(forward=this.js.forward, turn=this.js.turn){
		...
		let forwardValue = forward;
		let turnValue = turn;
	
		
		// Check if keyboard controls are active and update forward and turn values accordingly
		if (this.forwardKeyPressed) {
			forwardValue = -1;

		} else if (this.backwardKeyPressed) {
			forwardValue = 1;
		}
		...
		const force = maxForce * forwardValue;
		const steer = maxSteerVal * turnValue;
	...
	animate() {
		...
		if (this.world!==undefined){
			this.updateDrive(this.js.forward, this.js.turn);
